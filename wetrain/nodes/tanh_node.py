...
class tanh_node(cal_node):
    ...
    def forward(self):
        #计算节点值
        self.data_2.data = math.tanh(self.data_1.data)
        #设置节点状态
    ...
    def backward(self):
        ...
        if self.data_1.require_grad == True:
            self.data_1.grad += (1. - math.pow(math.tanh(self.data_1.data),2)) * self.data_2.grad
            self.data_1._node_attr['backward_type'] = 'tanh_backward'
        else:
            self.data_1.grad = 0.
...
if __name__ == '__main__':
    input1 = data_node(1.57)
    input2 = data_node()
    input2.grad = 0.4

    print(math.tanh(1.57))

    adder = tanh_node(input1, input2)
    adder.forward()
